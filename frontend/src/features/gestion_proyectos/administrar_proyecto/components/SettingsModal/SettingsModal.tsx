...
export const SettingsModal: React.FC<Props> = ({ onClose }) => {
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      try {
        ...
        setMessage({ type: 'error', text: 'Error al cargar la configuración de usuario.' });
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSaveDb = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!otpCode.trim()) {
      setMessage({ type: 'error', text: 'Por favor ingresa el código OTP recibido en tu correo.' });
      return;
    }
  ...
  return (
    <div className="settings-modal-overlay">
      <div className="settings-modal-card">
        ...
        <div className="settings-modal-scrollable">
          {message && (
          ...
          {loading ? (
            ...
            <div className="settings-sections">
              ...
              <div className="settings-card-section">
                <div className="section-header">
                  ...
                  <div>
                    ...
                    <p>Requerido para exportar repositorios de código Spring Boot generados por IA.</p>
                  </div>
                </div>

                {githubVinculado ? (
                  <div className="settings-badge-success">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>
                    <span>Conectado como <strong>@{githubUsername}</strong></span>
                  </div>
                ) : (
                  <button className="settings-btn-github" onClick={iniciarVinculacion}>
              ...
              {/* Sección 2: Contraseña de Base de Datos Host */}
              <div className="settings-card-section">
                <div className="section-header">
                  ...
                  <div>
                    ...
                    <p>Contraseña asignada a tu usuario PostgreSQL en Gerlextech Host. Se requiere verificación por código OTP enviado a tu correo.</p>
                  </div>
                </div>
                    ...
                    <>
                      ...
                      <div className="settings-actions">
                        ...
                        <button
                          ...
                          disabled={saving || !otpCode || !dbPassword}
                        >
                          {saving ? 'Guardando en Host...' : 'Actualizar Contraseña en Host'}
                        </button>
                      </div>
                    </>
                  )}
